...
import { ADD_PRODUCT, DELETE_PRODUCT, UPDATE_PRODUCT } from "../constants/actionType";
import { LOCAL_STORE_NAME_PRODUCT } from "../constants/localStorageName";

//hằng
let initState = [];
// kiểm tra xem mua lần đầu tiên hay mua thêm
const shoppingCartLocal = JSON.parse(localStorage.getItem(LOCAL_STORE_NAME_PRODUCT));
initState =
...
const cart = (state = initState, action) => {
  ...
  switch (action.type) {
    case ADD_PRODUCT: // mua hàng
      ...
      return [...state];
    case UPDATE_PRODUCT: // cập nhật giỏ hàng
      index = getIndexByProduct(state, product);
      if (index >= 0) {
      ...
      localStorage.setItem(LOCAL_STORE_NAME, JSON.stringify(state));
      return [...state];
    case DELETE_PRODUCT: // xoá sản phẩm trong giỏ hàng
